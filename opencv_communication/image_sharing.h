	...
	public:
		...
		virtual IReport detect(Mat& img_scene)
		{
			...
			for(unsigned idx = 0; idx < num_objects(); ++idx)
			{
				...
				object_tracker[idx].update(processObject(object_library().object_img_idx[idx], local_matches));
				debugImage(object_library().object_img_idx[idx], local_matches);
			}
		...
		std::vector<double> process_neighbor_msgs(std::list< std::vector<boost::asio::mutable_buffer> >& neighbor_msgs)
		{
			...
			for(std::vector<boost::asio::mutable_buffer>& msg : neighbor_msgs)
			{
				...
				for(unsigned idx = 0; idx < num_objects(); ++idx)
				{
					...
					likelihood[idx] += (double) processObject(object_library().object_img_idx[idx], neighbor_matches);
				}
			}
			return likelihood;
		}
